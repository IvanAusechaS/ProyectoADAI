from estrategia2 import EncuestaHeapABB, TemaABB, PreguntaHeap, Encuestado

def leer_archivo_txt_estrategia_2(ruta):
    with open(ruta, 'r', encoding='utf-8') as f:
        lineas = [line.rstrip("\n") for line in f]

    encuesta = EncuestaHeapABB()
    idx = 0
    id_auto = 1
    ...
    while idx < len(lineas):
        ...
        while idx < len(lineas) and lineas[idx].strip():
            ...
            if bloque.startswith("{"):
                ...
                pregunta = PreguntaHeap(f"{tema_num}.{pregunta_num}")
                for eid in ids:
                    if eid in encuesta.encuestados:
                        pregunta.agregar_encuestado(encuesta.encuestados[eid])
